...
class Galerias extends Model {
	 ...
	 * @var mixed
	 */
	public $IdGaleria, $Nombre, $Texto, $IdImagen;
	...
	public function Buscar() {
		...
		if ($this->Nombre !== null) {
			$aWhere[] = "Galerias.nombre = '{$this->Nombre}'";
		}
		if ($this->Texto !== null) {
			$aWhere[] = "Galerias.texto = '{$this->Texto}'";
		}
		if ($this->IdImagen !== null) {
			$aWhere[] = "Galerias.id_imagen = '{$this->IdImagen}'";
		}

		// Arma la cadena SQL.
		$cWhere = $aWhere ? 'WHERE '.join(' AND ', $aWhere) : '';
		
		$cSql = "SELECT SQL_CALC_FOUND_ROWS Galerias.* FROM Galerias {$cWhere} {$cOrderBy} {$cLimit}";
		$this->DB->Query($cSql);
	...
	public function Obtener() {
		if ($this->IdGaleria) {
			$cSql = "SELECT Galerias.* FROM Galerias WHERE Galerias.id_galeria = '{$this->IdGaleria}' LIMIT 1";
		}
	...
	public function Guardar() {
		...
		if ($this->IdGaleria) {
			$cSql = "UPDATE Galerias SET ".
				"nombre = '{$this->Nombre}', ".
				"texto = ".($this->Texto ? "'{$this->Texto}'" : 'NULL').", ".
				"id_imagen = ".($this->IdImagen ? "'{$this->IdImagen}'" : 'NULL')." ".
				"WHERE id_galeria = '{$this->IdGaleria}' LIMIT 1";
			
			$this->DB->Query($cSql);
		}
		else {
			$cSql = "INSERT INTO Galerias SET ".
				"nombre = '{$this->Nombre}', ".
				"texto = ".($this->Texto ? "'{$this->Texto}'" : 'NULL').", ".
				"id_imagen = ".($this->IdImagen ? "'{$this->IdImagen}'" : 'NULL');
			
			$this->IdGaleria = $this->DB->QueryInsert($cSql);
		}
	...
	public function Eliminar() {
		$cSql = "DELETE IGNORE FROM Galerias WHERE id_galeria = '{$this->IdGaleria}' LIMIT 1";
		$this->DB->Query($cSql);
	...
	private function Set($aDatos) {
		$this->IdGaleria = $aDatos['id_galeria'];
	...
	public function Limpiar() {
		$this->IdGaleria = $this->Nombre = $this->Texto = $this->IdImagen = null;
	}
}
?>
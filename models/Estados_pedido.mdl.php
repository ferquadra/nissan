<?
class Estados_pedido extends Model {
	 ...
	 * @var mixed
	 */
	public $IdEstadopedido, $Nombre, $Editable;
	...
	public function Buscar() {
		...
		if ($this->Nombre !== null) {
			$aWhere[] = "estados_pedido.nombre LIKE '%{$this->Nombre}%'";
		}
		if ($this->Editable !== null) {
			$aWhere[] = "estados_pedido.editable = '{$this->Editable}'";
		}
		
		// Arma la cadena SQL.
		$cWhere = $aWhere ? 'WHERE '.join(' AND ', $aWhere) : '';
		
		$cSql = "SELECT SQL_CALC_FOUND_ROWS estados_pedido.* FROM estados_pedido {$cWhere} {$cOrderBy} {$cLimit}";
		$this->DB->Query($cSql);
	...
	public function Obtener() {
		$cSql = "SELECT estados_pedido.* FROM estados_pedido WHERE estados_pedido.id_estadopedido = '{$this->IdEstadopedido}' LIMIT 1";
	...
	public function Guardar() {
		...
		if ($this->IdEstadopedido) {
			$cSql = "UPDATE estados_pedido SET ".
				"nombre = '{$this->Nombre}' ".
				"WHERE id_estadopedido = '{$this->IdEstadopedido}' LIMIT 1";
			
			$this->DB->Query($cSql);
		}
		else {
			$cSql = "INSERT INTO estados_pedido SET ".
				"nombre = '{$this->Nombre}'";
			
			$this->IdEstadopedido = $this->DB->QueryInsert($cSql);
		}
	...
	public function Eliminar() {
		$cSql = "DELETE IGNORE FROM estados_pedido WHERE id_estadopedido = '{$this->IdEstadopedido}' LIMIT 1";
		$this->DB->Query($cSql);
	...
	private function Set($aDatos) {
		$this->IdEstadopedido = $aDatos['id_estadopedido'];
		$this->Nombre = $aDatos['nombre'];
		$this->Editable = $aDatos['editable'];
	}
	...
	public function Limpiar() {
		$this->IdEstadopedido = $this->Nombre = $this->Editable = null;
	}
}
?>
...
class Bares extends Model {
	...
	public function Buscar() {
		...
		if ($this->Codigo !== null) {
			$aWhere[] = "bares.codigo = '{$this->Codigo}'";
		}
		if ($this->Nombre !== null) {
			$aWhere[] = "bares.nombre LIKE '%{$this->Nombre}%'";
		}
		if ($this->BusquedaRecursiva) { // Busca tambien en las categorias de niveles inferiores.
			if ($this->IdCategoria !== null) {
				...
				$aWhere[] = "bares.id_categoria IN (".join(', ', $aCategs).")";
			}
		}
		else {
			if ($this->IdCategoria !== null) { // Busca solo en las categorias de primer nivel.
				$aWhere[] = "bares.id_categoria = '{$this->IdCategoria}'";
			}
		}
		if ($this->Descripcion !== null) {
			$aWhere[] = "bares.descripcion = '{$this->Descripcion}'";
		}
		if ($this->Texto !== null) {
			$aWhere[] = "bares.texto = '{$this->Texto}'";
		}
		if ($this->IdImagen !== null) {
			$aWhere[] = "bares.id_imagen = '{$this->IdImagen}'";
		}
		if ($this->Publicado !== null) {
			$aWhere[] = "bares.publicado = '{$this->Publicado}'";
		}
		if ($this->Destacado !== null) {
			$aWhere[] = "bares.destacado = '{$this->Destacado}'";
		}
		if ($this->IdZona !== null) {
			$aWhere[] = "bares.id_zona = '{$this->IdZona}'";
		}
		if ($this->Where !== null) {
			$aWhere[] = $this->Where;
		}		
		if ($this->TerminoBusqueda !== null) {
			...
			$aWhere[] = "bares.nombre LIKE '%".join("%' AND bares.nombre LIKE '%", array_map('utf8_encode', array_map('addslashes', $aPalabras)))."%'";
		}
		
		switch ($this->PorLetra) {
			case 'numeros':
				$aWhere[] = "bares.nombre REGEXP '^[^a-z]'";
				break;
			...
			default:
				$aWhere[] = "bares.nombre LIKE '{$this->PorLetra}%'";
				break;
		}
		...
		$cSql = "SELECT SQL_CALC_FOUND_ROWS bares.*, categorias_producto.nombre categoria
			FROM bares LEFT JOIN categorias_producto ON categorias_producto.id_categoriaproducto = bares.id_categoria
	...
	public function Obtener() {
		...
		$cSql = "SELECT SQL_CALC_FOUND_ROWS bares.*, categorias_producto.nombre categoria, localidades.nombre localidad
			FROM bares LEFT JOIN categorias_producto ON categorias_producto.id_categoriaproducto = bares.id_categoria 
			LEFT JOIN localidades ON bares.id_localidad = localidades.id_localidad
			WHERE bares.id_bar = '{$this->Id}' LIMIT 1";
	
		//$cSql = "SELECT bares.* FROM inmuebles WHERE bares.id_inmueble = '{$this->IdProducto}' LIMIT 1";
	...
	public function Cargar() {
		$cSql = "SELECT bares.* FROM bares WHERE bares.codigo = '{$this->Codigo}' LIMIT 1";
	...
	/**
	 * @desc
	 * Despublica todos los bares.
	 *
	 * @return array
	 */
	public function Despublicar() {
		
		$cSql = "UPDATE bares SET bares.publicado = 0";
		...
		*/
		if ($this->Id) {
			$cSql = "UPDATE bares SET ".
				...
				"comidas = ".($this->Comidas ? "'{$this->Comidas}'" : 'NULL').", ".
				"caracteristicas = ".($this->Caracteristicas ? "'{$this->Caracteristicas}'" : 'NULL').", ".
				"servicios = ".($this->Servicios ? "'{$this->Servicios}'" : 'NULL').", ".
				"destacado = ".($this->Destacado ? '1' : '0')." ".
				"WHERE id_bar = '{$this->Id}' LIMIT 1";
			
			$this->DB->Query($cSql);
		}
		else {
			$cSql = "INSERT INTO bares SET ".
				"codigo = ".($this->Codigo ? "'{$this->Codigo}'" : 'NULL').", ".
				"nombre = '{$this->Nombre}', ".
				"id_categoria = ".($this->IdCategoria ? "'{$this->IdCategoria}'" : 'NULL').", ".
				"descripcion = ".($this->Descripcion ? "'{$this->Descripcion}'" : 'NULL').", ".
	...
	public function Eliminar() {
		$cSql = "DELETE IGNORE FROM bares WHERE id_bar = '{$this->Id}' LIMIT 1";
		$this->DB->Query($cSql);
	...
	private function Set($aDatos) {
		...
		$this->Comidas = $aDatos['comidas'];
		$this->Caracteristicas = $aDatos['caracteristicas'];
		$this->Servicios = $aDatos['servicios'];
	...
	public function Importar() {
		...
		// Lee todas las lineas e importa los bares.
		while ($aData = fgetcsv($sFp, null, $_POST['separador'], '"')) {
			...
			if ($aProducto['id_bar']) {
				$cSql = "UPDATE productos SET ".
					...
					"WHERE id_bar = '{$aProducto['id_bar']}' LIMIT 1";
				mysql_query($cSql, $sMY);
			}
			else {

 ...
 * DbDriver_MsSql
 * 
 * Driver de conexión a motores de bases de datos MsSql.
 * 
 ...
 */
class DbDriver_MsSql implements IDbDriver {
	...
	public function Initialize($cHost, $cUser, $cPass, $cDbName) {
		// Realiza la conexión.
		if (APP_DATABASE_PERSISTENT) $this->Link = @mssql_pconnect($cHost, $cUser, $cPass);
		else $this->Link = @mssql_connect($cHost, $cUser, $cPass);
	...
	private function RuntimeError($nErrNo) {
		...
		switch ($nErrNo) {
			case 1:
				error::Raise(E_WARNING, "Imposible establecer conexion con el servidor.<br >MsSql Dice: ".@mssql_get_last_message());
				break;
				
			case 2:
				error::Raise(E_WARNING, 'No se ha podido establecer la codificaci&oacute;n '.APP_DATABASE_CHARSET.' para la variable '.$aArgs[1].'<br />'.@mssql_get_last_message());
				break;
				
			case 3:
				error::Raise(E_WARNING, 'No se ha podido establecer la colaci&oacute;n '.APP_DATABASE_COLLATE.' para la variable '.$aArgs[1].'<br />'.@mssql_get_last_message());
				break;
				
			case 4:
				error::Raise(E_WARNING, 'No se puede abrir la base de datos '.$aArgs[1].'<br />'.@mssql_get_last_message());
				break;
				
			case 5:
				error::Raise(E_WARNING, 'Se produjo una falla en la instruccion '.$aArgs[1].'<br />'.@mssql_get_last_message());
				break;
			...
			case 7:
				error::Raise(E_WARNING, 'No se puede obtener la cantidad de registros devueltos por la ultima consulta SQL.<br />'.@mssql_get_last_message());
				break;
				
			case 8:
				error::Raise(E_WARNING, 'No se pudo realizar con exito la transaccion<br />'.@mssql_get_last_message());
				break;
				
			case 9:
				error::Raise(E_WARNING, 'Error al buscar el proximo ID autonumero en la tabla <u>'.$aArgs[1].'</u><br />'.@mssql_get_last_message());
				break;
				
			case 10:
				error::Raise(E_WARNING, 'Error al obtener la cantidad de filas afectadas en la ultima consulta SQL.<br />'.@mssql_get_last_message());
				break;
				
			case 11:
				error::Raise(E_WARNING, 'Error al obtener un registro.<br />'.($this->Link ? @mssql_get_last_message() : @mssql_get_last_message()));
				break;
			...
			default:
				error::Raise(E_ERROR, 'Error de MsSql indefinido.<br />'.@mssql_get_last_message());
	...
	public function Open($cBaseName=APP_DATABASE_NAME) {
		if(@mssql_select_db($cBaseName, $this->Link)) {
			return true;
		}
		else {
	...
	public function Query($cSql) {
		...
		$this->Result = @mssql_query($cSql, $this->Link);
		
		if ($this->StartedTransaction && !$this->Result) {
			$this->TransactionErrors[] = @mssql_get_last_message();
		}
	...
	public function Field($cCol=null) {
		$aField = @mssql_fetch_assoc($this->Result);
	...
	public function Count() {
		$nCount = @mssql_num_rows($this->Result);
	...
	public function AffectedRows() {
		$nRet = @mssql_affected_rows($this->Link);
		if ($nRet === null) {
	...
	/**
	 * <b>Método NextInsertID() no implementado en DbDriver_MsSql</b>.
	 *
	 * @param string $cTabla
	 * @return int
	 */
	public function NextInsertID($cTabla) {
		$this->RuntimeError(13, 'DbDriver_MsSql::NextInsertID()');
		return false;
	}
	...
	public function GetRecordset($cCol=null) {
    	...
    	if($cCol === null) {
    		...
    		if (APP_DATABASE_ADDSLASHES) {
				while($aField = @mssql_fetch_assoc($this->Result)) { $aRet[] = $this->AddSlashes($aField); }
    		}
    		else {
				while($aField = @mssql_fetch_assoc($this->Result)) { $aRet[] = $aField; }
    		}
		}
		else {
    		// El if para ver si tiene que escapar lo hace por fuera del bucle para mejorar la performance.
    		if (APP_DATABASE_ADDSLASHES) {
    			...
    			} while($aField = @mssql_fetch_assoc($this->Result));
    		}
    		else {
    			// Primero busca si está la columna, una sola vez, para mejorar la performance.
    			$aField = @mssql_fetch_assoc($this->Result);

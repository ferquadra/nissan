...
<tbody>
	...
	<?$item = escape($item)?>
		<tr class="<?=$pos % 2 == 1 ? 'impar' : ''?>">
			<td width="20"><a href="" class="publish" clave_primaria="<?=$item['id_anuncio']?>" title="Publicado"><img src="images/<?=$item['publicado'] ? '' : 'dis/'?>ok_16.png" /></a></td>
			<td width="20"><a href="?p=anuncios|editar&id_anuncio=<?=$item['id_anuncio']?>" title="Modificar"><img src="images/write_16.png" /></a></td>
			<td><?=$item['nombre']?></td>
			<td><?=$item['url']?></td>
			<td><?=$item['vigencia_desde'] ? mysql2date($item['vigencia_desde']) : 'XX/XX/XXXX'?> a <?=$item['vigencia_hasta'] ? mysql2date($item['vigencia_hasta']) : 'XX/XX/XXXX'?></td>
			<td><a href="" class="delete" clave_primaria="<?=$item['id_anuncio']?>" title="Borrar"><img src="images/delete_16.png" /></a></td>
		</tr>
	<?endforeach;?>
</tbody>
</table>
...
$j().ready(function() {
	$j(".admin-list").preparar_listado({data: {url: "?p=anuncios"}});

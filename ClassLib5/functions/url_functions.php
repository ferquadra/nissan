...
if (! function_exists('slug')) {
	function slug($cadena, $separador = '-') {
		...
		// Convierto de UTF8 a ASCII usando el modo TRANSLIT,
		// que convierte por ejemplo:
		//  �    en    EUR
		// (ver documentacion de iconv en PHP)
		$limpia = iconv('UTF-8', 'ASCII//TRANSLIT', $limpia);
		// Reemplazo todo lo que NO sea A-Z, a-z, 0-9, /,
		// y todos los +, -, y espacios a ""
		$limpia = preg_replace("/[^a-zA-Z0-9\/_|+ -]/", '', $limpia);
		// recorto guiones que puedan haber quedado al final
		// y convierto a minuscula
		$limpia = strtolower(trim($limpia, '-'));
		// reemplazo /, _, +, - y espacio, por el separador
		$limpia = preg_replace("/[\/_|+ -]+/", $separador, $limpia);
